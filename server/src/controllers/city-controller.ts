...
class CityController {
    ...
    public convert() {
        let input = `https://image.ibb.co/dcW7pJ/sunvalley_thumb.jpg
    ...
    https://image.ibb.co/j4XNpJ/niagara_falls_thumb.jpg`;
        ...
        let inputArray = inputColons.split(',');
        let json: any = [];
        inputArray.forEach(function (element) {
            let reg = /.*\//g;
            let imgName = element.replace(reg, "");
            let nameArray = imgName.split("_");
            let name = "";
            let route_name = "";
    ...
    public deleteCity(route_name: string) {
        return new Promise((resolve, reject) => {
            return this.getCityByName(route_name)
                .then(result => {
                    return City.deleteOne({ route_name }, (err) => {
                        if (err) {
